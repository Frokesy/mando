...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white/95 backdrop-blur-xl shadow-[0_-8px_30px_rgba(0,0,0,0.08)]">
      <div className="mx-auto flex max-w-4xl justify-around px-2 py-3 sm:px-4">
        {tabs.map((tab) => {
          ...
          return (
            <Link
              key={tab.href}
              href={tab.href}
              className={`relative flex flex-col items-center gap-1 rounded-2xl px-2.5 py-2 transition-all duration-200 sm:px-4 ${
